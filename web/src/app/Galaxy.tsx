    ...
    <div className="relative w-full h-full select-none">
      ...
      <div className="absolute top-3 right-3 flex flex-col items-end gap-2 z-10">
        <div className="flex items-center gap-1 rounded-xl bg-[#10101a]/85 backdrop-blur border border-white/10 p-1 text-[11px]">
          ...
          <button onClick={() => { setMode('radar'); setMotion('still'); setFlow(false); setDimOff(new Set()) }} title="8 chiều liên kết cân hay lệch — bấm trục để soi riêng" className={`px-2 py-1.5 rounded-lg ${mode === 'radar' ? 'bg-fuchsia-500/35 text-fuchsia-100 shadow-lg shadow-fuchsia-500/30' : 'text-zinc-400 hover:bg-white/10'}`}>🎯 Radar</button>
        ...
        <div className="flex items-center gap-1 rounded-xl bg-[#10101a]/85 backdrop-blur border border-white/10 p-1 text-[11px]">
          ...
          <button onClick={() => setSugOpen(s => !s)} title="Máy soi cặp trang nên nối mà chưa nối" className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg ${sugOpen ? 'bg-fuchsia-500/30 text-fuchsia-100' : 'text-zinc-400 hover:bg-white/10'}`}><span className="dq-prev-sug">✨</span>Gợi ý</button>
        </div>
      </div>
